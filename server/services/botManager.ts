  ...
  static async startBot(initialConfig: BotConfig) {
    ...
    try {
      ...
      client.on('messageCreate', async (message: any) => {
        ...
        // .timestamp {elapsed_seconds} {remaining_seconds}
        if (command === 'timestamp') {
            const elapsed = parseInt(args[0]) || 0;
            const remaining = parseInt(args[1]);
            
            if (isNaN(remaining)) {
                await message.edit("Usage: .timestamp <elapsed_seconds> <remaining_seconds>").catch(() => {});
                return;
            }
            ...
            client.user?.setActivity(rpc);
            await message.edit(`Timestamp updated: ${elapsed}s elapsed, ${remaining}s remaining.`).catch(() => {});
            return;
        }
